    ...
    <body>
        ...
        <main>
            ...
            <?php
                ...
                $all_boba_query = "SELECT `BobaFlavour` FROM `boba`";
                $all_boba_results = mysqli_query($dbcon, $all_boba_query);

                /* Display all price for each boba flavour */
                $all_price_query = "SELECT `price` FROM `boba`";
                $all_price_results = mysqli_query($dbcon, $all_price_query);

                while ($all_boba_row = mysqli_fetch_assoc($all_boba_results)) {
                    echo "<br>";
                    // show boba flavours in the product layout //
                    echo "<option value ='".$all_boba_row['BobaFlavour'] ."'>";
                    echo $all_boba_row['BobaFlavour'];
                    echo "</option>";

                }
                while($all_price_row = mysqli_fetch_assoc($all_price_results)) {
                    // show price for each boba flavour in the product layout //
                    echo "<option value ='".$all_price_row['price'] ."'>";
                    echo $all_price_row['price'];   // Show the price for each boba flavour
                    echo "</option>";
                    echo "<br>";
                }
            ?>
